...
if __name__ == "__main__":
	...
	# convert the `last_name` column in both REDCap and HealthPro records to lowercase
	# the reason is because JOIN is case-sensitive
	hp_recs['last_name'] = hp_recs['last_name'].str.lower()
	rc_recs['last_name'] = rc_recs['last_name'].str.lower()

	# create a LEFT JOIN between HealthPro records and exisiting AoU records
	# columns on which we have performed the JOIN are `pmi_id`, `dob`, `last_name`
	hp_rc_merged = pd.merge(hp_recs, rc_recs, left_on=['pmi_id', 'dob', 'last_name'], 
											  right_on=['pmi_id', 'dob', 'last_name'], 
											  how='left')
	...
	hp_rc_not_nulls['hp_last_name'] = hp_rc_not_nulls['hp_last_name'].str.capitalize()
	print(hp_rc_not_nulls)
	# hp_rc_not_nulls.to_csv('records_matched_pmi_id.csv', index=False)
	...
	hp_rc_nulls['hp_last_name'] = hp_rc_nulls['hp_last_name'].str.capitalize()
	print(hp_rc_nulls)
	# hp_rc_nulls.to_csv('nulls_pmi_id.csv', index=False)
